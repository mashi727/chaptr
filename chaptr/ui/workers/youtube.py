...
def _candidate_app_dirs() -> List[Path]:
    """同梱バイナリを探索するためのアプリケーションディレクトリ候補"""
    dirs: List[Path] = []
...
def resolve_ytdlp_exe() -> Optional[str]:
    """外部yt-dlp実行ファイルのパスを解決

    1. PATH上のyt-dlp（macOS/Linuxのbrew/pipインストール等）
    2. アプリ実行ファイルと同じディレクトリ
       （Windows配布パッケージに同梱されるyt-dlp.exe）
    """
...
class YouTubeDownloadWorker(QThread, CancellableWorkerMixin):
    ...
    def get_ytdlp_strategy() -> Tuple[str, str, str]:
        ...
        """
        # 同梱版（yt_dlpモジュール）は環境によっては存在しない
        # （例: Windowsのビルドは別途yt-dlp.exeを同梱する）
        bundled_ver = ""
        try:
            import yt_dlp
        ...
        external_ver = ""
        external_path = resolve_ytdlp_exe()
        if external_path:
            try:
                result = subprocess.run(
                    [external_path, '--version'],
        ...
        # 同梱版が無い場合は外部版を使う（外部版も無ければ実行時にエラー）
        if not bundled_ver:
    ...
    def _run_external(self):
        ...
        ytdlp_exe = resolve_ytdlp_exe() or 'yt-dlp'
        cmd = [
            ytdlp_exe,
            # H.264優先、AV1除外（macOSでハードウェアデコード非対応のため）
