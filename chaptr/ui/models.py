...
# チャプタータイトル用フォント（プレビューと焼き込みで共通の単一情報源）
OVERLAY_FONT_FILENAME = "NotoSansJP-Bold.otf"
OVERLAY_FONT_FAMILY = "Noto Sans JP"


def get_overlay_font_path() -> str:
    """チャプタータイトルの焼き込み／プレビュー共通フォントの絶対パスを返す。

    リポジトリ同梱の Noto Sans JP Bold を単一情報源とし、プレビュー(Qt)と
    焼き込み(ffmpeg drawtext)で同一フォントを使うことで WYSIWYG を保証する。
    開発環境と PyInstaller バンドルの双方に対応。同梱フォントが見つからない
    場合は detect_system_font() にフォールバックする。
    """
    if getattr(sys, "frozen", False):
        # PyInstallerバンドル
        base = Path(sys._MEIPASS) / "chaptr" / "ui" / "fonts"
    else:
        # 開発環境: models.py は chaptr/ui/ 配下 → 同階層の fonts/
