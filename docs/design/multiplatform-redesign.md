...
### 4.1 画面レイアウトと動画プレビューの位置づけ

「動画再生画面はどこに入るか」への回答。**動画は精密作業の主役ではなく、音声マスタークロックに
追従する確認用プレビュー**として配置する。境界（曲の切れ目・指揮の振り下ろし等）を**目で確かめる**役割で、
ms 精度の打点は音声タイムライン（波形/スペクトログラム）が担う。

#### 同期モデル（重要）

- **マスター = 音声クロック**（`PlayerClock`, サンプル精度）。音は音声エンジンから出す。
- **スレーブ = 動画**。動画要素は**ミュート**し、表示だけを音声クロックに追従させる。
  - スクラブ/一時停止中（精密作業）: 再生ヘッド ms の**静止フレーム**を表示（フレーム抽出 or `video.currentTime` 代入）。ここが精度の要だが、1フレーム seek で十分成立。
  - 連続再生中: 動画をミュート再生し、ズレたら currentTime / playbackRate で**緩く補正**（±1 フレーム同期で可）。
- これにより「ms 精度の再生制御」（音声）と「視覚確認」（動画）を**両立**できる。音声を二重に鳴らさないため動画は常にミュート。

#### デバイス別レイアウト

```
[デスクトップ / iPad横 — 横長]                 [iPad縦 / iPhone — 縦長・省スペース]
┌──────────── header: transport ───────────┐  ┌──── header(最小) ────┐
├────────────┬──────────────────────────────┤  │  動画プレビュー(折畳)  │ ← トグルで表示/縮小
│  動画       │  波形 (時間–振幅)            │  ├──────────────────────┤
│  プレビュー │──────────────────────────────┤  │  波形 / スペクトロ     │ ← 単一表示を切替
│  (16:9,可変)│  スペクトログラム            │  │                      │
├────────────┴──────────────────────────────┤  ├──────────────────────┤
│  時間ルーラー + チャプター一覧             │  │  チャプター一覧(別ペイン)│
└────────────────────────────────────────────┘  └──────────────────────┘
```

- **横長（精密編集機）**: 動画は**左（or 右）の可変・折畳可能なペイン**。タイムラインを広く保つため全幅にはしない。
- **縦長（iPhone等）**: 動画は**上部の折畳サムネイル**。レビュー用途中心（UX評価 §4 の役割分担に整合）。
- **補助案: フィルムストリップ・トラック**。波形直下に一定間隔のサムネイル列を敷くと、大きな動画ペイン無しで
  視覚的な切れ目を一望できる。チャプター打点ツールと相性が良い（任意機能）。

#### プラットフォーム実装（動画デコード）

| PF | 動画プレビュー手段 | 備考 |
|---|---|---|
| デスクトップ | ffmpeg / ネイティブ or `<video>` | フレーム抽出は ffmpeg でも可 |
| iOS/iPadOS | `AVPlayer`(ミュート) / `AVAssetImageGenerator` | **ffmpeg不要**・ネイティブ。静止フレーム抽出に最適 |
| Android | `ExoPlayer` / `MediaMetadataRetriever` | 同上 |
| Web/Tauri webview | `<video muted>` 要素 | currentTime を音声ヘッドへ追従 |

> 留意: Web/PoC で「Web Audio 再生 + `<video>` 表示」を別々に持つと**二重デコード**になる。
> 本番（Flutter/Swift/Tauri-Rust）では、解析用 PCM（波形/STFT）と表示用デコードを分離しつつ、
> 音は音声エンジン一本に集約する。

### 4.2 ソース取り込み（SourceProvider 抽象）

ローカルに加え **YouTube リンク / Dropbox 等のリモート** に対応したい、という要件への設計。
鍵となる原則は1つ:

> **波形・スペクトログラム・ms再生はいずれも「実際にデコードした音声」を必要とする。**
> したがってどのソースも最終的に**ローカルにバイト列（または range 取得可能なストリーム）を落として
> デコード・キャッシュする**経路に集約する。埋め込み再生だけのソース（YouTube iframe 等）は
> 解析パイプラインに食わせられない＝本ツールの中核機能を満たせない。

#### SourceProvider インターフェース

```
SourceProvider.resolve(origin) -> ResolvedMedia {
  localHandle,         // 解析・デコード用のローカルファイル/バイト列（必要に応じDL+キャッシュ）
  kind: audio|video,   // 動画なら音声トラックを解析、映像はプレビューへ
  durationMs, hash     // content-hash で peaks/STFT タイルをキャッシュ（再オープン即時・オフライン可）
}
```

下流（`PcmProvider` → 波形/STFT/再生、動画プレビュー）はすべて **resolve 後のローカルハンドル**を消費する。

#### ソース × プラットフォーム 成立性マトリクス

| ソース | デスクトップ | iOS / iPadOS | Android | Web / PWA |
|---|---|---|---|---|
| **ローカルファイル** | ◎ ピッカー/D&D | ◎ Document Picker | ◎ SAF | ◎ File API |
| **クラウド（Dropbox/iCloud/Drive/OneDrive）** | ◎ 同期フォルダ/アプリ | **◎ ファイルアプリ(File Provider)から透過アクセス＝追加実装ほぼ不要** | ◎ SAF が各プロバイダを統合 | △ 直リンク+CORS / 各社チューザ |
| **汎用 HTTPS URL** | ◎ DL/range | ◎ URLSession でDL+キャッシュ | ◎ | △ CORS 依存（range/CORS可なら○）|
| **YouTube リンク** | ◎ **yt-dlp（既存実装あり）** | ⚠ **ストア規約5.2.3で配布不可** | ⚠ Google Play も同様 | ⚠ CORS＋規約 |

#### 推奨（段階導入）

- **Tier 1（全PF・低コスト）= ローカル ＋ クラウドは「ファイル」経由。**
  Dropbox/iCloud/Drive/OneDrive は **OS のドキュメントピッカー経由で“ローカルファイルと同じ扱い”**になる
  （iOS=File Provider, Android=SAF, デスクトップ=同期フォルダ）。**「Dropbox 等リモート対応」は専用 API/OAuth なしで
  ローカルピッカーがそのまま満たす**。まずここを完成させる。
- **Tier 2 = 汎用 HTTPS URL。** 直リンクを DL してキャッシュ→解析。Web のみ CORS に注意（range/CORS 許可が要る）。
  必要に応じて Dropbox/Drive の API（OAuth）で「アプリ内ブラウズ＋ストリーミング」を足すのは将来拡張。
- **Tier 3（要注意）= YouTube。**
  - **デスクトップ**: 既存の yt-dlp 経路（`youtube_mixin.py` / `workers/youtube.py`）がそのまま使える。◎
  - **モバイル/ストア配布**: **Apple 5.2.3・Google Play・YouTube ToS により、アプリ内 YouTube ダウンロードは不可**。
    現実解は (a) **デスクトップ限定機能**にする / (b) **「自分でDLした動画ファイルを取り込む」運用**（=Tier 1 に帰着）/
    (c) 自分の権利あるコンテンツ向けに**自前バックエンドで音声抽出して配信**（個人/許諾範囲のみ）。
  - 配布前提なら **YouTube は“デスクトップ＆個人利用”に留め、iOS アプリの売り機能にしない**のが安全。

> まとめ: 「Dropbox 等リモート」は **OS のファイル統合で実質コストゼロで成立**。「YouTube」は **デスクトップでは成立・
> モバイルのストア配布では規約上の壁**がある、という非対称性を前提に段階導入する。
...
| スペクトログラム | ● | | |
| ms 再生制御 | ● | | |
| **動画プレビュー（静止フレーム／ミュート追従）** | ● | | |
| 動画の連続再生（音声マスターに緩く同期） | ●(任意) | ● | |
...
| YouTube取り込み（モバイル/ストア配布） | リスク（規約） | Apple5.2.3/Google Play/YouTube ToS で不可。デスクトップ限定 or 取り込み運用に留める（§4.2 Tier3）。 |
| リモート（Dropbox等）対応 | 解決済方針 | OSのファイル統合（File Provider/SAF/同期フォルダ）でローカル扱い。専用API不要（§4.2 Tier1）。 |
...
- App Store 審査ガイドライン 5.2.3（YouTube 等からのDL/変換を許可なく行うアプリは不可）:
  https://developer.apple.com/app-store/review/guidelines/#intellectual-property
- iOS「ファイル」/ File Provider（Dropbox/Drive/OneDrive を透過アクセス）:
  https://support.apple.com/en-us/102238 , https://developer.apple.com/documentation/FileProvider
